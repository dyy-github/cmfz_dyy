...
<!doctype html>
    ...
    <script type="text/javascript">
        $(function () {
            $("#dg").datagrid({
                fit:true,
                url:"${pageContext.request.contextPath}/banner/bannerPaging",
                    pagePosition:"bottom",
                pageList:[2,4,6,8],
                pageSize:4,
                height:400,
                ...
                columns:[[
                    ...
                    {title:'排序',field:'descs',fixed:false,width:100},
                    {title:'状态',field:'status',fixed:false,width:100},
                    {title:'日期',field:'date',fixed:false,width:100}
                ]],
                view: detailview,
        ...
        function xixi() {
            $("#ff").form('submit',{
                url:'${pageContext.request.contextPath}/banner/bannerAdd',
        ...
        function xg() {
            ...
            if (rows.length==1){
            $("#update").dialog({
                href:'${pageContext.request.contextPath}/back/banner/update.jsp?id='+rows[0].id,
                buttons:[
                    {
                        text:'修改',
                        iconCls:'icon-save',
                        handler:function () {
                            $("#save1form").form('submit',{
                                url:'${pageContext.request.contextPath}/banner/bannerModify',
                                success:function (result) {
                                    //var r=$.parseJSON(result);
                                    if(result.success){
                                        /*提示信息*/
        ...
        function pl() {
            ...
            if (rows.length<=0){
                ...
                console.log(ids);
                $.ajax({
                    url:'${pageContext.request.contextPath}/banner/bannerP',
                    type:'POST',
                    traditional:true,
                    data:{id:ids},
                    success:function (result) {
