...
<!doctype html>
    ...
    <script type="text/javascript">
        $(function () {
            $("#gdg").datagrid({
                fit:true,
                url:"${pageContext.request.contextPath}/guru/guruPaging",
                    pagePosition:"bottom",
                pageList:[2,4,6,8],
                pageSize:4,
                height:400,
                ...
                columns:[[
                    ...
                    {title:'姓名',field:'name',fixed:false,width:100},
                    {title:'头像',field:'headPic',fixed:false,width:100},
                    {title:'性别',field:'gender',fixed:false,width:100}
                ]],
                view: detailview,
        ...
        function bannerSave() {
            $("#dl").dialog({
                href:'${pageContext.request.contextPath}/back/guru/guruAdd.jsp',
                title:'部门添加',
                iconCls:'icon-save',
                width:400,
                height:400,
        ...
        function xixi() {
            $("#ff").form('submit',{
                url:'${pageContext.request.contextPath}/guru/guruAdd',
        ...
        /*修改对话框*/
        function xg(id) {
            var rows=$("#gdg").datagrid('getSelections');
            if (rows.length==1){
            $("#update").dialog({
                href:'${pageContext.request.contextPath}/back/guru/guruUpdate.jsp?id='+rows[0].id,
                buttons:[
                    {
                        text:'修改',
                        iconCls:'icon-save',
                        handler:function () {
                            $("#save1form").form('submit',{
                                url:'${pageContext.request.contextPath}/guru/guruModify',
                                success:function (result) {
                                    //var r=$.parseJSON(result);
                                    if(result.success){
                                        /*提示信息*/
        ...
        function pl() {
            ...
            if (rows.length<=0){
                ...
                console.log(ids);
                $.ajax({
                    url:'${pageContext.request.contextPath}/guru/guruP',
                    type:'POST',
                    traditional:true,
                    data:{id:ids},
                    success:function (result) {
